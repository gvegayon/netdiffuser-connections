...
Another thing to note are the warnings. The R programming language (like most do) distinguishes between decimal ("`numeric`") and integer numbers. In this case, the function is throwing a warning about columns beign coerced from numeric into integer types. These are mostly informative, and if the user rather skipping these, they can use the `suppressWarnings()` function in R. We will see more warnings as we execute R code and will pause at some points to discuss further about what these mean to the user.
...
Since exposure can be computed for each time point, the `exposure()` function will return a matrix of size $n \times \text{}$ the number of time points in the data; in the previous example, we had a glimpse of the exposure for the first ten individuals at time point five.

The function also prints two other messages. The first, a warning about the `value` parameter beign swithced to `TRUE`, indicates that, since we are using the structural equivalence alternative graph, instead of treating the network as unweighted (0/1-valued ties), the function will compute exposures using weights. The second is a more technical internal message that is describing a change in the `Matrix` R package: users should not worry about this particular message.[^dgcmatrix]
...
The warnings in `exposure()` indicate that (a) the passed graph is going to be avaluated as a valued (weighted) network and (b) the same graph is going to be used to compute exposure in all time points (so it is static). The next code chunk gives a glimpse of the different exposure values we have just calculated:
...
Starting with version 1.23.0, **netdiffuseR** supports simulating multi- and dis-adoption diffusion processes. To study a multi-adoption process, you can pass a `list` as the `seed.p.adopt` parameter. Here is a simple example using the Koren Family Planning network:
