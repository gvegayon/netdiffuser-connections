...
Since exposure can be computed for each time point, the `exposure()` function will return a matrix of size $n \times \text{}$ the number of time points in the data; in the previous example, we had a glimpse of the exposure for the first six individuals at time point five.
