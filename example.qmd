...
::: {.callout-tip}
You can always inspect what datasets are included in an R package using the `data()` function. For example, in netdiffuseR, you can use `data(package="netdiffuseR")`.
:::
...
*netdiffuseR* also can handle adoption processes for more than one behavior. To select a multiadoption process, you can pass a `list` as the `seed.p.adopt` parameter. Here is a simple example using the Koren Family Planning network:
...
Inspecting the output from the print method of the `diffnet` object, we can see that the object contains two behaviors: The  "Num of behaviors" entry now shows `2`, the "Behavior" entry also shows two behaviors, `"Random contagion \_1, Random contagion\_2"`, and finally, the "Prevalence" entry also shows two numbers: `0.92, 0.96`. Although we are simulating two behaviors, `rdiffnet` will simulate as many as values are in the `seed.p.adopt` list. 

::: {.callout-tip}
In the current implementation of `rdiffnet`, the multiadoption module simulates behaviors independently. That is, the code above would be equivalent to simulating the same behavior twice. More complicated models in which behaviors are interdependent are supported via the disadoption parameter.
:::
...
In this particular example, we ran `rdiffnet` with most of the parameters being `list`. The reader is invited to look at other types of possible inputs in the `rdiffnet` documentation.
...
To visualize the diffusion process when there's more than one behavior, we can use the `split_behaviors` function to split the diffnet object into a list of diffnet objects, one for each behavior. Then, we can use the `plot_adopters` function to visualize the diffusion process for each behavior:

```{r}
#| label: simulation-multiadoption-process-7-plot
diffnets <- split_behaviors(diffnet)
plot_adopters(diffnets[[1]])
```
